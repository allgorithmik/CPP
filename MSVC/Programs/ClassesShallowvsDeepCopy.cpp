...
//Member wise copy of member variables, including pointers                          When a pointer member variables are involved, allocating new memory and copying in
//                                                                                  data from the source pointer
...
class Person{
    ...
    public:
        Person() = default;
        Person(std::string l_name_param);
        Person(std::string f_name_param, std::string l_name_param);
...
//All these syntax denote shallow copying
...
//THIRD ATTEMPT => PASS OBJECT BY REFERENCE => THIS COPIES THE DEREFERENCED DATA OF THE POINTER INTO A NEW HEAP MEMORY LOCATION => CORRECT WAY OF DOING THINGS
//age(new int(*(source_person.get_age()))) syntax denotes deep copying => creates a new memory location and copies the data to that new location
